...
import org.openpnp.machine.reference.feeder.ReferenceBankFeeder.Bank;
import org.openpnp.machine.reference.feeder.ReferenceBankFeeder.Feeder;
...
public class ReferenceBankFeederConfigurationWizard
        extends AbstractConfigurationWizard {
    private final ReferenceBankFeeder feeder;
    ...
    private JComboBox comboBoxPosActuator;
    private JTextField posActuatorValue;
    private JButton btnTestFeedActuator;
    private JButton btnTestPostPickActuator;
    private JButton btnMoveActuatorToPos;
    ...
    public ReferenceBankFeederConfigurationWizard(ReferenceBankFeeder feeder) {
        this.feeder = feeder;
        ...
        JLabel lblBankPos = new JLabel("Selector Position");
        panelActuator.add(lblMoveBeforeFeed, "2, 10, right, default");

        comboBoxPosActuator = new JComboBox();
        comboBoxPosActuator.setModel(new ActuatorsComboBoxModel(Configuration.get().getMachine()));
        panelActuator.add(comboBoxPosActuator, "4, 10, fill, default");

        posActuatorValue = new JTextField();
        posActuatorValue.setColumns(10);
        panelActuator.add(posActuatorValue, "6, 10");

        btnMoveActuatorToPos = new JButton(testPostPickActuatorAction);
        panelActuator.add(btnTestPostPickActuator, "10, 10");
        ...
        for (Bank bank : ReferenceBankFeeder.getBanks()) {
            bankCb.addItem(bank);
        }
        feederCb.addItem(null);
    ...
    private Action newBankAction = new AbstractAction("New") {
        ...
        public void actionPerformed(ActionEvent e) {
            Bank bank = new Bank();
            ReferenceBankFeeder.getBanks().add(bank);
            bankCb.addItem(bank);
    ...
    private Action testGoToPickupLocAction = new AbstractAction("Move Bank to pickup pos") {
        @Override
        public void actionPerformed(ActionEvent arg0) {
            UiUtils.submitUiMachineTask(() -> {
                if (feeder.getPostPickActuatorName() == null || feeder.getPostPickActuatorName().equals("")) {
                    Logger.warn("No testGoToPickupLocAction specified for feeder {}.", feeder.getName());
                    return;
                }
                Actuator actuator = Configuration.get().getMachine()
                        .getActuatorByName(feeder.getPostPickActuatorName());
