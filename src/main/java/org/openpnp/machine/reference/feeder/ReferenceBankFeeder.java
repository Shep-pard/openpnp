...
import org.openpnp.machine.reference.feeder.wizards.ReferenceBankFeederConfigurationWizard;
...
public class ReferenceBankFeeder extends ReferenceAutoFeeder {
    ...
    public static synchronized IdentifiableList<Bank> getBanks() {
        BanksProperty bp = (BanksProperty) Configuration.get().getMachine().getProperty("ReferenceBankFeeder.banks");
        if (bp == null) {
            bp = new BanksProperty();
            Bank bank = new Bank();
            bank.setName("Default");
            bp.banks.add(bank);
            Configuration.get().getMachine().setProperty("ReferenceBankFeeder.banks", bp);
        }
        return bp.banks;
    }
    ...
    public Wizard getConfigurationWizard() {
        return new ReferenceBankFeederConfigurationWizard(this);
    }

    @Root
    public static class Bank extends AbstractModelObject implements Identifiable, Named {
        @ElementList
        private IdentifiableList<ReferenceBankFeeder.Feeder> feeders = new IdentifiableList<>();
        ...
        BiMap<Feeder, ReferenceBankFeeder> assignments = HashBiMap.create();
        ...
        public void setFeeder(ReferenceBankFeeder slot, Feeder feeder) throws Exception {
            assignments.forcePut(feeder, slot);
        }
        
        public Feeder getFeeder(ReferenceBankFeeder slot) {
    ...
    public static class Feeder extends AbstractModelObject implements Identifiable, Named {
        ...
        private ReferenceBankFeeder owner = null;
        ...
        public void setOwner(ReferenceBankFeeder owner) throws Exception {
            if (this.owner == owner) {
                return;
            }
            if (this.owner != null) {
